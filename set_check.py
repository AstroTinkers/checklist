...
while True:
    ...
    if input_data == "q":
        if len(check_set) == 0:
            ...
            final_list = sorted(check_set, key=lambda item: (item.isnumeric(), int(item) if item.isnumeric() else item))
            # item - checks every entry if it's integer or strings and sorts accordingly
